...
function renderNoticiaDetalhes(noticia) {
  ...
  if (noticia.categories && noticia.categories.length > 0) {
    ...
    category.textContent = noticia.categories[0].charAt(0).toUpperCase() + noticia.categories[0].slice(1);
  ...
  author.innerHTML = `<i class="fas fa-user"></i> ${noticia.author || 'Equipe InvestSavy'}`;
  meta.appendChild(author);

  // Data
  const date = document.createElement('span');
  date.className = 'noticia-date';
  date.innerHTML = `<i class="far fa-calendar"></i> ${formatDate(noticia.created_at)}`;
  meta.appendChild(date);

  // Tempo de leitura
  const readingTime = document.createElement('span');
  readingTime.className = 'noticia-reading-time';
  readingTime.innerHTML = `<i class="far fa-clock"></i> ${noticia.reading_time || '5 min de leitura'}`;
  meta.appendChild(readingTime);

  // Visualizações
  const views = document.createElement('span');
  views.className = 'noticia-views';
  views.innerHTML = `<i class="far fa-eye"></i> ${noticia.views || '0'} visualizações`;
  ...
  featuredImage.style.backgroundImage = `url('${noticia.image || '/images/default-news.jpg'}')`;
  ...
  facebookLink.href = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`;
  ...
  twitterLink.href = `https://twitter.com/intent/tweet?url=${encodeURIComponent(window.location.href)}&text=${encodeURIComponent(noticia.title)}`;
  ...
  linkedinLink.href = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.href)}`;
  ...
  whatsappLink.href = `https://api.whatsapp.com/send?text=${encodeURIComponent(noticia.title + ' ' + window.location.href)}`;
...
async function loadNoticiasRelacionadas(noticia) {
  try {
    ...
    if (noticia.categories && noticia.categories.length > 0) {
      relacionadas = allNoticias.filter(item =>
        item.id !== noticia.id &&
        item.categories &&
    ...
    if (relacionadas.length < 3) {
      const outrasNoticias = allNoticias.filter(item =>
        item.id !== noticia.id &&
...
function renderNoticiasRelacionadas(noticias) {
  ...
  noticias.forEach(noticia => {
    ...
    imageDiv.style.backgroundImage = `url('${noticia.image || '/images/default-news.jpg'}')`;

    // Adicionar categoria se existir
    if (noticia.categories && noticia.categories.length > 0) {
      const categorySpan = document.createElement('span');
      categorySpan.className = 'news-category';
      categorySpan.textContent = noticia.categories[0].charAt(0).toUpperCase() + noticia.categories[0].slice(1);
    ...
    excerpt.textContent = noticia.excerpt || noticia.content.substring(0, 120) + '...';
    ...
    dateSpan.innerHTML = `<i class="far fa-calendar"></i> ${formatDate(noticia.created_at)}`;

    // Tempo de leitura
    const timeSpan = document.createElement('span');
    timeSpan.innerHTML = `<i class="far fa-clock"></i> ${noticia.reading_time || '5 min'}`;
...
async function loadSidebar() {
  try {
    ...
    const maisLidasNoticias = await window.supabaseClient.fetchNoticiasMaisLidas(5);
...
function renderUltimasAtualizacoes(noticias) {
  ...
  noticias.forEach(noticia => {
    ...
    meta.textContent = `${noticia.categories ? noticia.categories[0] + ' • ' : ''}${formatTime(noticia.created_at)}`;

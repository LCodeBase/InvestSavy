...
const ArtigoView = () => {
  ...
  // Função para obter a URL atual do artigo
  const getCurrentUrl = () => {
  ...
  const shareOnFacebook = () => {
    ...
    const text = artigo?.title || 'Confira este artigo interessante!';
  ...
  const shareOnLinkedIn = () => {
    const url = getCurrentUrl();
    const title = artigo?.title || 'Artigo Interessante';
    const summary = artigo?.description || 'Confira este conteúdo sobre educação financeira!';
  ...
  const shareOnWhatsApp = () => {
    const url = getCurrentUrl();
    const text = `${artigo?.title || 'Confira este artigo interessante!'} ${url}`;
